<div class="footer">

<div class="grid-container">
    <div class="grid-items">
    <h3>Project Bodima</h3>
    <p style="color:  rgb(180, 180, 180) ;font-weight:400;font-size:16px">
        bodima Accomadation Management System is multi facilitated platform form finding boarding places, advertising boarding places,<br/>advertising food delivery businesses<br/>and boarding places Managment.
    </p>
    </div>
    <div class="grid-items">
        <h3>Contact Us</h3>
        <ul>
            <li><i style="color: blue;" class="fa fa-map-marker fa-lg"></i>  35 Reid Ave, Colombo 7</li>
            <li><i style="color: blue;" class="fa fa-phone fa-lg"></i>  +94 0119999999</li>
            <li><i style="color: blue;" class="fa fa-paper-plane fa-lg"></i>  Projectbodima@gmail.com</li>
        </ul>
    </div>
    <div class="grid-items">
        <h3>About Us</h3>
        <ul>
            <li><i style="color: blue;" class="fa fa-user fa-lg"> </i>  User</li>
        </ul>
    </div>

</div>
<div class="follow">
    <h2>Follow Us</h2>
   <div class="social">
    <a href="index.php"><i class="fab fa-linkedin-in fa-2x"></i></a>
    <a href="index.php"><i class="fab fa-facebook-f fa-2x"></i></a>
    <a href="index.php"><i class="fab fa-twitter fa-2x"></i></a>
<a href="index.php"><i class="fab fa-whatsapp fa-2x"></i></a>
   </div>
</div>
<div class="copyright">
    <h2>© 2023 copyright all right reserved</h2>
</div>
</div>