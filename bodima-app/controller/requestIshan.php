...
session_start ();
include('../models/studentRequestModel.php');
...
if(isset($_GET['requestDelete_id'])){
   ...
   $result=StudentRequestModel::PendingrequestDelete($connection,$request_id);
   if($result){
...
if(isset($_GET['ArequestDelete_id'])){
   ...
   $result=StudentRequestModel::PendingrequestDelete($connection,$request_id);
   if($result){
      header('Location:../views/acceptBoardingRequestView.php');
   }
...
if(isset($_GET['requestCAccept_id'])){
   ...
   $result=StudentRequestModel::confirmDealAcc($request_id,$connection);
   if($result){
...
if (isset($_GET['requesttimeout_id'])) {
 ...
 $result=StudentRequestModel::setTimeoutIshanSt($request_id,$connection);
  if($result){
     header('Location:../views/pendingReqIshan_New.php');
   }
...
if ($result) {
  header('Location:boarderListController.php?boarderlist=1');
}
}
...
$result=StudentRequestModel::setHandover($connection,$request_id);
if ($result) {
...
$student_email=$_SESSION['email'];
  ...
  $Bid=$user['Bid'];
    ...
    StudentRequestModel::deleteconfirm($connection,$request_id);
    StudentRequestModel::deleteboaParent($connection,$Bid);
    StudentRequestModel::deleteboarder($connection,$Bid);
     StudentRequestModel::UpdateCtoAReq($connection,$request_id);
    header('Location: ../views/acceptBoardingRequestView.php');
