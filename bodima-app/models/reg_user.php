...
// boarding owner registration 
    ...
    {
        $query="SELECT level,email,first_name,last_name,address,user_accepted FROM  student WHERE email='$email' AND password='$password' AND user_accepted=1 
        ...
        UNION SELECT level,email,first_name,last_name,address,user_accepted FROM boarder  WHERE email='$email' AND password='$password' AND user_accepted=1 
        LIMIT 1 ";
        $result_set=mysqli_query($connection,$query);
        return  $result_set;
    }
